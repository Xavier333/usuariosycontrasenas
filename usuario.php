...
*/
    ...
    #borramos el registro de la base 
        $sql ="DELETE FROM `usuarios` WHERE `usuarios`.`id_usuarios` =".$idUsuario; 
        #$sql="INSERT INTO `proyectos` (`descripcion`) VALUES ('$show');";
        $id_consultas = $conexion->ejecutar($sql);
    /*
    else {
        $sql="INSERT INTO `proyectos` (`descripcion`) VALUES ('El usuario esta en uso.');";
        $id_consultas = $conexion->ejecutar($sql);
    }
    */
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        $conexion = new conexion();
                        $cantDeUso = $conexion->consultar("SELECT COUNT(*) FROM proyectos WHERE proyectos.usuario =".$idUsuario);                        
                        ?>
                        <td><?php echo $cantDeUso[0][0];?></td>
                        <?php
                            if($cantDeUso[0][0] < 1 ) { 
                        ?>
                            <td><a name="eliminar" id="eliminar" class="btn btn-danger" href="?borrar=<?php echo $usuario['id_usuarios'];?>">Eliminar</a></td> 
                        <?php       
                            } 
                        ?>
                        <!--
